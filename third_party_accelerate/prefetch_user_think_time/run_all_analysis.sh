...
run_analysis() {
  ...
  prefetch_resource_used_output_dir=${page_output_dir}/prefetch_resource_used
  mkdir -p "${prefetch_resource_used_output_dir}"
  mkdir -p "${page_output_dir}"/resource_count
  # Find prefetched resources used.
  echo "python find_prefetched_resources_used.py ${page_outgoing_links_dir}/0/ --output-resource-count="${page_output_dir}"/resource_count/data > ${page_output_dir}/prefetch_resource_used/data"
  python find_prefetched_resources_used.py "${page_outgoing_links_dir}/0/" --output-resource-count="${page_output_dir}"/resource_count/data > "${page_output_dir}/prefetch_resource_used/data"
  echo "python find_prefetched_resources_used.py ${page_outgoing_links_dir}/0/ --output-resource-count="${page_output_dir}"/resource_count/data --group-pages > ${page_output_dir}/prefetch_resource_used/data_grouped"
  python find_prefetched_resources_used.py "${page_outgoing_links_dir}/0/" --output-resource-count="${page_output_dir}"/resource_count/data --group-pages > "${page_output_dir}/prefetch_resource_used/data_grouped"
}
