...
def main(config_filename, pages, iterations, device_name, mode, output_dir):
    failed_pages = []
    not_completed_pages = set([ x[0] for x in pages ])
    try:
        ...
        while len(not_completed_pages) > 0:
            for run_index in range(0, iterations):
                for page_tuple in pages:
                    ...
                    if page_to_tries_counter[page] > MAX_LOAD_TRIES:
                        ...
                        not_completed_pages.remove(page)
                        continue

                    # Only want to load the pages that are not completed
                    if page not in not_completed_pages:
                        continue
                    ...
                    # Load the page.
                    returned_page, timed_out_index = load_one_website(redirected_page, run_index, output_dir, device_info, mode, replay_configurations, current_time)
                    ...
                    if args.fetch_server_side_logs:
                        ...
                        iteration_output_dir = os.path.join(output_dir, 'server_side_logs', str(run_index))
                        if not os.path.exists(iteration_output_dir):
                            os.mkdir(iteration_output_dir)
                        fetch_server_side_logs(escaped_page, iteration_output_dir, replay_configurations)
...
def fetch_server_side_logs(page, output_dir, replay_configurations):
    output_filename = os.path.join(output_dir, page)
    server_hostname = replay_configurations[replay_config_utils.SERVER_HOSTNAME]
    if server_hostname.startswith('ec2'):
        command = 'scp -i ~/.ssh/vaspol_aws_key.pem ubuntu@{0}:~/build/logs/{1} {2}'.format(server_hostname, page, output_dir)
    elif server_hostname.startswith('apple-pi'):
        command = 'scp {0}:~/Research/MobileWebOptimization/page_load_setup/build/logs/{1} {2}'.format(server_hostname, page, output_dir)
