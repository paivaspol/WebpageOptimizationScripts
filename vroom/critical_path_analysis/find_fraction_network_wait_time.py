...
if len(sys.argv) < 4:
    print 'usage: [dep_tree] [timing] [dependency_filename] [network_wait_time_timings]'
    sys.exit(1)

dep_tree = sys.argv[1]
timing = sys.argv[2]
dependency_filename = sys.argv[3]
network_wait_time_timings_filename = sys.argv[4]
...
def parse_network_wait_times(network_wait_time_timings_filename):
    result = {}
    with open(network_wait_time_timings_filename, 'rb') as input_file:
        for raw_line in input_file:
            line = raw_line.strip().split()
            result[line[0]] = int(line[1])
    return result

def compute(dep_tree, timing, dependencies, network_wait_time_timings_filename):
    results = {}
    object_mappings = {}
    ...
    timings = {}
    # read in timing info
    network_wait_times = parse_network_wait_times(network_wait_time_timings_filename)
    ...
    for path in root_to_leaf_paths:
        ...
        for node in path:
            if ( node in timings ):
                ...
                tmp_net_delay = network_wait_times[node]
    ...
    return critical_path, critical_path_time, critical_path_net_delay
...
critical_path, critical_path_time, critical_path_net_delay = compute(dep_tree, timing, dependencies, network_wait_time_timings_filename)
# print 'critical path:'
# print critical_path
fraction_critical_path_net_delay = 1.0 * critical_path_net_delay / critical_path_time
# print 'fraction critical path net delay: {0}'.format(fraction_critical_path_net_delay)
print fraction_critical_path_net_delay
