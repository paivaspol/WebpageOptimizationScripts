...
def main(data_dir, pages_filename, dependency_dir):
    ...
    for page_tuple in pages:
        ...
        if not (os.path.exists(dep_tree_filename) and \
            ...
            cmd = '[ERROR] find_fraction_of_wait_time_of_dynamic_resources_on_critical_path.py {0} {1} {2}'.format(dep_tree_filename, timing_filename, dependency_filename)
            print cmd
            continue

        # Find network wait-time fraction.
        ...
        # Find processing time fraction
        cmd = 'python find_fraction_of_processing_time_of_dynamic_resources_on_critical_path.py {0} {1} {2}'.format(dep_tree_filename, timing_filename, dependency_filename)
        proc = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = proc.communicate()
        if not stdout.startswith('max') and len(stdout.strip().split(' ')) > 0:
            print '{0} {1}'.format(page, stdout.strip())
