...
def compute(dep_tree, timing, dependencies):
    ...
    critical_path_processing_time = -1
    critical_path_net_delay = -1
    for path in root_to_leaf_paths:
        dynamic_resource_processing_time = 0
        ...
        for node in path:
            if ( node in timings ):
                if 'jquery' in node or '622640' in node or 'ga40ee28' in node:
                    print timings[node]
                ...
                tmp_processing_time = 0
                if len(timings[node][constants.TRACING_PROCESSING_TIME]) == 2:
                    tmp_processing_time = timings[node][constants.TRACING_PROCESSING_TIME][1] - timings[node][constants.TRACING_PROCESSING_TIME][0]

                processing_time += tmp_processing_time
                if node not in dependencies:
                    dynamic_resource_processing_time += max(0, tmp_processing_time)
        ...
        # print str(path) + ' ' + str(finish_time)

        if critical_path_time < finish_time:
            critical_path_time = finish_time
            critical_path = path
            critical_path_processing_time = processing_time
            critical_path_net_delay = net_delay
            critical_path_not_hinted = num_dynamic_resources
            critical_path_dynamic_wait_time = dynamic_resource_processing_time
            critical_path_dynamic_net_delay = dynamic_resource_network_delay

    return critical_path, critical_path_time, critical_path_not_hinted, critical_path_dynamic_wait_time, critical_path_processing_time, critical_path_net_delay, critical_path_dynamic_net_delay
...
critical_path, critical_path_time, critical_path_not_hinted, critical_path_dynamic_processing_time, critical_path_processing_time, critical_path_net_delay, critical_path_dynamic_net_delay = compute(dep_tree, timing, dependencies)
print 'critical path:'
print critical_path
fraction_processing = 1.0 * critical_path_dynamic_processing_time / critical_path_time
fraction_net_delay = 1.0 * critical_path_dynamic_net_delay / critical_path_time
print '{0} {1} {2} {3}'.format(critical_path_dynamic_processing_time, critical_path_time, fraction_processing, fraction_net_delay)
...
fraction_critical_path_processing_time = 1.0 * critical_path_dynamic_processing_time / critical_path_processing_time
print 'fraction critical path processing: {0}'.format(fraction_critical_path_processing_time)
fraction_critical_path_net_delay = 1.0 * critical_path_dynamic_net_delay / critical_path_net_delay
print 'fraction critical path net delay: {0}'.format(fraction_critical_path_net_delay)

# Sum (1) and (2)
total = critical_path_dynamic_processing_time + critical_path_dynamic_net_delay
fraction_total = 1.0 * total / critical_path_time
print 'Total: {0}'.format(fraction_total)
