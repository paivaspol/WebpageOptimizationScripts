...
def main(root_dir, output_dir):
    for path, dirs, filenames in os.walk(root_dir):
        ...
        escaped_path = url if url.endswith('_') else url[:len(url) - 1]
        page_output_dir = os.path.join(output_dir, escaped_path)
        if not os.path.exists(page_output_dir):
            os.mkdir(page_output_dir)
        command = 'python find_cpu_utilization.py {0} {1} {2}'.format(cpu_utilization_filename, interval_filename, page_output_dir)
